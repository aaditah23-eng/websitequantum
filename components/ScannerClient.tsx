...
export default function ScannerClient({ initialResult }: { initialResult: ScanResult }) {
  ...
  const pqcLevel3Label = result.pqc?.pqcLevel3Supported
    ? 'Level 3 detected'
    : result.pqc?.pqcLevel3Tested
      ? 'Not detected'
      : 'Demo / not tested';
  ...
  return (
    <>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 py-16 lg:grid-cols-[0.9fr_1.1fr] lg:py-24">
          <div>
            ...
            <p className="mt-7 max-w-2xl text-lg leading-8 text-zinc-400">
              Scan public domains for TLS readiness, certificate posture, security headers,
              DNS email security, and real Level 3 post-quantum TLS negotiation.
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {['stripe.com', 'cloudflare.com', 'openai.com', 'github.com'].map((item) => (
                <button
                  key={item}
                  onClick={() => runScan(item)}
                  className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-zinc-300 hover:border-emerald-400/40 hover:bg-emerald-400/10 hover:text-emerald-200"
                >
                  Scan {item}
                </button>
              ))}
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-4 shadow-2xl shadow-emerald-950/30 backdrop-blur-xl md:p-6">
            <div className="rounded-[1.5rem] border border-white/10 bg-black/70 p-5">
              <div className="mb-5 flex items-center justify-between">
                ...
                <div className="text-xs text-zinc-500">/api/scan</div>
              </div>
              ...
              <div className="rounded-3xl border border-emerald-400/20 bg-emerald-400/[0.06] p-4">
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium">Domain Scanner</div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  <button
                    onClick={() => runScan()}
                    disabled={loading}
                    className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-400 px-5 py-3 text-sm font-semibold text-black transition hover:bg-emerald-300 disabled:opacity-60"
                  >
                    {loading ? <Icon name="loader" size={17} /> : <Icon name="radar" size={17} />}
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <MiniMetric label="PQC Level 3" value={pqcLevel3Label} />
              </div>
              ...
              {result.id && (
                <a
                  ...
                  className="mt-5 flex items-center justify-center gap-2 rounded-2xl border border-white/10 px-4 py-3 text-sm text-zinc-200 hover:bg-white/10"
                >
                  Open saved report <Icon name="arrow" size={16} />
                </a>
              )}
            </div>
          </div>
      ...
      <section id="report" className="relative mx-auto max-w-7xl px-5 pb-24 md:px-8">
